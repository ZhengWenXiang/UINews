...
- (UIScrollView *)titleScrollView
{
    if (_titleScrollView == nil) {
        ...
        titleScrollView.backgroundColor = _titleScrollViewColor?_titleScrollViewColor:[UIColor colorWithWhite:0.7 alpha:0.1];
        
        [self.contentView addSubview:titleScrollView];
//        titleScrollView.backgroundColor = [UIColor whiteColor];
//        titleScrollView.alpha = 0.6;
...
#pragma mark -> 改变标题宽度
        CGFloat width = titleBounds.size.width+50;
...
{
    // 获取文字尺寸
    CGRect titleBounds = [label.text boundingRectWithSize:CGSizeMake(MAXFLOAT+0, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil];
    
    CGFloat underLineH = _underLineH?_underLineH:YZUnderLineH;
    
    self.underLine.y = label.height - underLineH - 3;
    self.underLine.height = underLineH;
    
    // 最开始不需要动画
    if (self.underLine.x == 0) {
        self.underLine.width = titleBounds.size.width+16;
        
        self.underLine.x = label.x+16;
        return;
    }
